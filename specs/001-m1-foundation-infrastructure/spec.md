...
3. **Given** multiple consumers requesting the same collection, **When** each requests an instance, **Then** the underlying connection is shared — not duplicated — and each consumer receives an equivalent, usable instance. → T023 (identity assert: `provider._client is provider._client`)
...
3. **Given** OpenRouter credentials in configuration, **When** an OpenRouter provider instance is requested, **Then** a ready-to-use LLM instance is returned that routes through OpenRouter (no `OPENROUTER_BASE_URL` configuration key is used in M1; the dedicated `ChatOpenRouter` class manages the endpoint internally).
...
1. **Given** traces exist in Langfuse, **When** traces are queried by bot identifier, session identifier, or date range, **Then** all matching trace records are returned as structured objects with input, output, and metadata — each query method (`get_by_bot`, `get_by_session`, `get_by_date_range`) is independently usable.
...
- **Empty string config key**: A configuration key present in `.env` or `config/*.yaml` with an empty string value is treated identically to a missing key — `ConfigManager` raises an FR-003 descriptive error naming the key and its expected source file.
- **Key collision `.env` vs YAML**: When both `.env` and `config/settings.yaml` define the same key with different values, the `.env` value wins silently — standard dotenv precedence convention. No error is raised; the YAML value is discarded for that key.
- **Invalid vector collection name**: When the collection name contains characters outside `^[a-zA-Z0-9_-]+$`, `QdrantVectorStoreProvider` raises `VectorStoreError` immediately — before any network call — with a message naming the invalid name and the allowed pattern.
- **Langfuse trace with `output=None`**: When a trace represents an interrupted session with no bot response, `TraceRecord.output` is `None`. `TraceRepository` returns the record as-is without raising or filtering. The calling evaluation pipeline (future milestone) is responsible for handling `None` output (e.g., skip scoring or apply a fallback).
- **DB available but target table missing**: When Supabase is reachable but `evaluation_results` does not exist (migration not applied), the Supabase SDK raises an exception on insert. `EvaluationRepository` catches it and re-raises as `RepositoryError` — the same FR-017 path. The original SDK message ("relation does not exist") is preserved in the error to aid diagnosis.
- **LLM provider auth error mid-evaluation**: When the provider API returns an authentication error during a completion call (e.g., key revoked after instantiation), the LangChain SDK exception propagates naturally to the caller — `LLMProviderBase` does not catch or wrap runtime auth failures. Credential validation at instantiation time (FR-011) cannot prevent post-instantiation revocations; runtime error handling is the responsibility of the evaluation pipeline (future milestone). *(Integration-level test for this scenario deferred to M2.)* <!-- M2 -->
...
- **FR-007a**: `LangfuseClient.submit()` MUST map each `TelemetryEvent` to a single `langfuse.trace()` call using the SDK's low-level API. When `TelemetryEvent.trace_id` is `None`, a new trace is created; when it is set, the existing Langfuse trace is updated (`langfuse.trace(id=trace_id, ...)`). Fields map as follows: `name→name`, `session_id→session_id`, `input→input`, `output→output`, `metadata→metadata`, `start_time→start_time`, `end_time→end_time`. No `span()`, `generation()`, or `event()` primitives are used in M1.
- **FR-007b**: When the Langfuse SDK raises an exception during `LangfuseClient.__init__()`, the singleton MUST log a `WARNING` (including the exception detail), set `is_connected()` to return `False`, and continue without re-raising — the process MUST NOT crash because the observability platform is unreachable at startup. `submit()` calls made while `is_connected()` is `False` follow the same warning-and-continue behaviour defined in FR-007. `LangfuseError` is exported as part of the public API but is never raised internally in M1 (both paths log WARNING and continue — no exception propagates to the caller). Future milestones that introduce irrecoverable failure paths MUST raise `LangfuseError` explicitly at that point; in M1 there is no scenario where a caller needs to catch it.
- **FR-008**: The system MUST expose vector store instances for named collections through a unified interface; collections MUST be created automatically if they do not exist. The embedding model and its dimensions MUST be sourced from `config/settings.yaml` via `ConfigManager` — a single global model applies to all collections in V1. If a requested collection already exists in Qdrant with a different number of embedding dimensions than the globally configured value, the system MUST raise `VectorStoreError` immediately — reusing a collection with mismatched dimensions would silently corrupt semantic search results. The caller is responsible for using distinct collection names per embedding configuration. If `embedding.model` or `embedding.dimensions` is absent or has an empty string value in `config/settings.yaml`, the system MUST follow the FR-003 path: raise `ConfigError` naming the missing key and the expected source file.
...
- **FR-011**: LLM provider instantiation MUST source all credentials and endpoint configuration from `ConfigManager`; no credentials may be accepted as parameters from external callers. Providers MUST retrieve credentials internally from `ConfigManager` and MUST pass them to the underlying LangChain SDK constructor — this is the correct and expected internal flow, not a violation of this rule.
...
- **FR-015**: The evaluation persistence layer MUST write metric scores, evaluation results, and associated metadata to the relational database. Each result MUST be assigned a UUID primary key generated by the application (via `uuid.uuid4()`) before the database insert, so the identifier is known to the caller without waiting for a database response. Date-based query parameters (e.g. `get_by_bot(date=...)`) MUST be UTC — naive `datetime` objects are treated as UTC; timezone-aware values that are not UTC MUST be rejected with a `ValueError`. The Supabase connection MUST be established using two credentials sourced from `ConfigManager`: `SUPABASE_URL` (the project HTTP/REST endpoint) and `SUPABASE_SERVICE_KEY` (the service-role key, obtained from the Supabase dashboard under Project Settings → API → service_role); both are required — absence of either MUST raise `ConfigError` via the standard FR-003 path.
...
- **TokenUsage**: A structured record of token consumption for a single LLM completion, containing `input_tokens` and `output_tokens` counts. Returned alongside the generated text by `LLMProviderBase.generate()` and `a_generate()`. Only defined as a custom type if no native equivalent exists in the installed `deepeval` package (Gate 5 — see T036a).
...
- **SC-005**: A complete evaluation round-trip (read trace → score with LLM judge → persist result) completes successfully in a local environment without requiring code changes beyond configuration file population. In M1, "run evaluation" means invoking `LLMProviderFactory.create()` to obtain an LLM judge and calling `generate()` to produce a score — full DeepEval metric invocation (`AnswerRelevancyMetric`, etc.) is out of scope for M1 and will be wired in a future milestone.
...
- **SC-007**: Telemetry events submitted during an evaluation run are visible in the Langfuse observability platform within 30 seconds of submission under normal operating conditions *(normal = Langfuse platform reachable, network round-trip < 500 ms, no active back-pressure from the SDK queue)*. *(Manual SLA verification only — not automatically enforced. Verified by the developer during Quickstart Scenario 2: after `flush()` returns, open the Langfuse dashboard and confirm the submitted trace appears within 30 seconds. Automated enforcement is deferred to a future milestone when infrastructure monitoring is in place.)*
...
- The Supabase project is already created and the target schema (evaluation results table) will be created as part of this milestone. `EvaluationRepository` (V1) connects via the Supabase Python SDK (`supabase>=2.0.0`) using its table/insert/select API; the Repository pattern isolates all SDK calls within the repository so a future V2 swap to self-hosted Postgres requires changes only to `EvaluationRepository`. Two distinct Supabase credentials are required: `SUPABASE_URL` (the project's HTTP/REST endpoint, used by the Python SDK at runtime) and `DATABASE_URL` (the direct PostgreSQL connection string in the form `postgresql://postgres:[password]@[host]:5432/postgres`, used only by `psql` or `supabase db push` to apply DDL migrations — never read by application code). Both must be present in `.env`; find `DATABASE_URL` in the Supabase dashboard under Project Settings → Database → Connection String → URI.
- Python `^3.11` is the minimum runtime version; `^3.13` is the pinned version for local development via `uv`.
- OpenRouter is accessed via the dedicated LangChain integration `ChatOpenRouter` from the `langchain-openrouter` package; the `ChatOpenAI + base_url` workaround MUST NOT be used — LangChain V1 docs explicitly recommend the dedicated integration over the OpenAI-compatible endpoint approach (constitution Principle II, plan.md Gate 5).
...
- Q: When a required configuration key exists but has an empty string value, should `ConfigManager` treat it as missing (raise FR-003 error) or as a valid empty string? → A: Empty string treated as missing — raise FR-003 error (same path as absent key). → See FR-003.
- Q: When Langfuse is unreachable, should `LangfuseClient` implement custom retry logic, buffer events indefinitely, discard immediately, or delegate to the SDK? → A: Delegate entirely to the Langfuse SDK's built-in async flush/retry mechanism — no custom retry layer. → See FR-007.
- Q: How should the embedding model for `QdrantVectorStoreProvider` be configured — global default, per-collection, or caller-provided? → A: Single global default in `config/settings.yaml`; all V1 collections use the same model and dimensions. → See FR-008.
