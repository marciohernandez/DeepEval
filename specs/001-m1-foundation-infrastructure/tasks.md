...
- [ ] T002 Add M1 runtime dependencies to `pyproject.toml` via: `uv add python-dotenv PyYAML langfuse langchain-qdrant langchain-openai langchain-anthropic "langchain-openrouter>=0.2" "supabase>=2.0.0" "deepeval>=4.0.6" "langchain>=1.3.10,<2.0.0" "langgraph>=1.2.6,<2.0.0"` — explicit pins for `langchain` and `langgraph` are required to satisfy the constitution V1 constraint; without them, transitive resolution may pull in 0.x versions
