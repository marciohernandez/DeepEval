"""Unit tests for LangChainExtractor (US1 — Trace Collection)."""
...
def _make_record(output) -> TraceRecord:
    """LangChain traces may nest output as {"messages": [...]}; presence/absence
    of output is still what determines completed vs. interrupted (spec.md)."""
    return TraceRecord(
        trace_id="trace-1",
        session_id="sess-1",
        bot_id="my-bot",
...
class TestLangChainExtractorIsSubclass:
    def test_is_trace_extractor_base_subclass(self):
        assert issubclass(LangChainExtractor, TraceExtractorBase)


class TestLangChainExtractorFiltering:
    def test_status_completed_returns_only_records_with_output(self):
        records = [
            _make_record({"messages": [{"role": "assistant", "content": "done"}]}),
            _make_record(None),
        ]
        result = LangChainExtractor().extract(records, InteractionStatus.COMPLETED)
        assert len(result) == 1
    ...
    def test_status_interrupted_returns_only_records_without_output(self):
        ...
        result = LangChainExtractor().extract(records, InteractionStatus.INTERRUPTED)
        assert len(result) == 1
    ...
    def test_status_none_returns_all_records(self):
        ...
        result = LangChainExtractor().extract(records, None)
        assert len(result) == 2

    def test_empty_input_returns_empty_list(self):
        result = LangChainExtractor().extract([], InteractionStatus.COMPLETED)
        assert result == []

    def test_empty_input_returns_empty_list_no_filter(self):
        result = LangChainExtractor().extract([], None)
        assert result == []
